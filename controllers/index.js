...
document.querySelector("#btnThemSV").onclick = function () {
    ...
    var valid = true;
    // Kiểm tra độ dài
    valid &= kiemTraDoDai(sv.matKhau, "#err__matkhau", " Mật Khẩu", 6, 32);
    // Kiểm tra rỗng
    valid &=
        kiemTraRong(sv.maSV, "#required__err__ma", "Mã Sinh Viên") &
        kiemTraRong(sv.tenSV, "#required__err__ten", "Tên Sinh Viên");
    // Kiểm tra định dạng
    var ngaySinhTest = moment(ngaySinh).format("YYYY-MM-DD");
    
    valid &=
        kiemTraTatCaKyTu(sv.tenSV, "#all__letter", "Tên Sinh Viên") &
        kiemTraTatCaLaSo(sv.diemToan, "#err_diemToan", "Điểm Toán") &
        kiemTraTatCaLaSo(sv.diemLy, "#err_diemLy", "Điểm lý") &
        kiemTraTatCaLaSo(sv.diemHoa, "#err_diemHoa", "Điểm Hóa") &
        kiemTraEmail(sv.emailSV, "#err__mail", "Email") &
        kiemTraMatKhau(sv.matKhau, "#err__matkhau", "Mật Khẩu") &
        kiemTraNgayThangNam(ngaySinhTest, "#err_date", "Ngày Sinh");
    // Kiểm tra giá trị
    valid &=
        kiemTraValue(sv.diemToan, "#err_diem_max_min_toan", "Điểm Toán", 0, 10) &
        kiemTraValue(sv.diemLy, "#err_diem_max_min_ly", "Điểm Lý", 0, 10) &
        kiemTraValue(sv.diemHoa, "#err_diem_max_min_hoa", "Điểm Hóa", 0, 10);
    // Kiểm tra biến cờ
    ...
    for (var index = 0; index < arrSinhVien.length; index++) {
        ...
        if (!sv.hasOwnProperty("tinhDiemTrungBinh")) {
            sv.__proto__.tinhDiemTrungBinh = function () {
                ...
                var diemTB = 0;
                diemTB =
                    (Number(sv.diemToan) + Number(sv.diemHoa) + Number(sv.diemLy)) / 3;
                return diemTB;
            };
        }
        ...
        var tr = `
             <tr>
                ...
                <td>
                    ...
                    <button class ="btn btn-primary" onclick ="suaSinhVien('${sv.maSV
            }')"> Sửa </button>
                    <button class ="btn btn-danger ml-2" onclick ="xoaMaSinhVien('${sv.maSV
            }')">  Xóa Mã Sinh Viên </button>
...
function xoaMaSinhVien(maSinhVien) {
    ...
    for (var index = mangSinhVien.length - 1; index >= 0; index--) {
        ...
        if (sv.maSV == maSinhVien) {
            // Nếu obj sinh viên trong mảng == mã sinh viên được click thì lấy ra vị trí
